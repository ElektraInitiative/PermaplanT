...
print "SQL queries:"
grep 'LOG:  execute s.*: SELECT \* FROM calculate_heatmap' postgis_parsed.log -A 2 | grep 'LOG:  duration: .* ms$' | awk '{sum+=$7; count++; min=10000; if ($7<0+min) min=$7; if ($7>0+max) max=$7} END {print "total:", count, "\nmin:", min, "ms\navg:", sum/count, "ms\nmax:", max, "ms"}'
