...
CREATE TYPE public.deciduous_or_evergreen AS ENUM ('deciduous', 'evergreen');
...
CREATE TYPE public.fertility AS ENUM ('self fertile', 'self sterile');
...
CREATE TYPE public.soil_water_retention AS ENUM ('well drained', 'moist', 'wet');
...
CREATE TYPE public.water_requirement AS ENUM ('Dry', 'Moist', 'Wet', 'Water');
...
CREATE FUNCTION public.diesel_manage_updated_at(_tbl regclass) RETURNS void LANGUAGE plpgsql AS $$ BEGIN EXECUTE format(
...
CREATE FUNCTION public.diesel_set_updated_at() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF (
    NEW IS DISTINCT
    FROM OLD
...
CREATE SEQUENCE public.plant_detail_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
CREATE SEQUENCE public.seeds_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
...
ALTER COLUMN id
SET DEFAULT nextval('public.plant_detail_id_seq'::regclass);
--
...
ALTER COLUMN id
SET DEFAULT nextval('public.seeds_id_seq'::regclass);
--
