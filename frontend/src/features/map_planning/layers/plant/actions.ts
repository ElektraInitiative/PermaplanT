...
/*
export class UpdatePlantingNotesPlantAction implements
Action<
  Awaited<ReturnType<typeof updateRemoveDatePlanting>>,
  Awaited<ReturnType<typeof updateRemoveDatePlanting>>
>{
  constructor (
    private readonly _data: Omit<UpdatePlantingNotesActionPayload, 'userId' | 'actionId'>,
    public actionId = v4(),
  ) {}
  ...
  reverse(state: TrackedMapState) {
    ...
    return new UpdatePlantingNotesPlantAction(
      {
        id: plant.id,
    ...
    const updatePlants = (plants: Array<PlantingDto>) => {
      return plants.map((p) => {
        if (p.id === this._data.id) {
          return {
            ...p,
            notes: this._data.plantingNotes,
          };
        }
  ...
  execute(mapId: number): Promise<PlantingDto> {
    return updateRemoveDatePlanting(mapId, this._data.id, {
      plantingNotes: this._data.plantingNotes,
